...
**Wichtiger Hinweis:** Aufgrund des UNC-Pfad-Problems (`\\ITME-SERVER\Projekte\ElectroVault`) können Tests derzeit nicht direkt im Netzwerkpfad ausgeführt werden.

**Workaround:**
1. Repository auf lokalen Pfad klonen (z.B. `C:\Dev\ElectroVault`)
...
### 1. UNC-Pfad-Kompatibilität ⚠️

**Problem:** Windows CMD kann nicht mit UNC-Pfaden als aktuellem Verzeichnis arbeiten.
...
**Impact:**
- Dependencies sind installiert (im `.pnpm`-Verzeichnis)
- Binaries können nicht direkt ausgeführt werden
- Tests funktionieren nicht vom Netzwerkpfad

**Lösungen:**
1. **Empfohlen:** Projekt auf lokalen Pfad klonen (z.B. `C:\Dev\ElectroVault`)
2. **Alternative:** Netzlaufwerk zuordnen (`net use Z: \\ITME-SERVER\Projekte`)
3. **CI/CD:** Funktioniert in GitHub Actions (kein UNC-Pfad)

### 2. Supertest Deprecation Warning ⚠️
