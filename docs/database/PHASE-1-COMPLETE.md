...
1. **pnpm install ausführen** (nur auf lokalem Pfad, nicht UNC)
   ```bash
   pnpm install
   ```
...
### 1. UNC-Pfad-Problem ⚠️

**Problem:** Migration kann nicht vom Netzwerkpfad (`\\ITME-SERVER\...`) ausgeführt werden.

**Lösung:** Projekt auf lokalen Pfad klonen (z.B. `C:\Dev\ElectroVault`)

### 2. Keycloak-Setup erforderlich ⚠️
