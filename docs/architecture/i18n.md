...
**Grundprinzipien:**
1. **Pflichtfelder brauchen mindestens eine Sprache** - Bei Pflichtfeldern (z.B. `name`) muss mindestens ein Wert gesetzt sein. Optionale Felder (z.B. `description`) können leer bleiben.
...
|------|---------|--------|-----------|
| `en` | English | ✅ Aktiv | Fallback-Sprache (DEFAULT_UI_LOCALE) |
| `de` | Deutsch | ✅ Aktiv | - |
| `fr` | Français | ✅ Aktiv | - |
| `es` | Español | ✅ Aktiv | - |
| `zh` | 中文 | ✅ Aktiv | Chinesisch (vereinfacht) |
| `ja` | 日本語 | ✅ Aktiv | Japanisch |
| `ko` | 한국어 | ✅ Aktiv | Koreanisch |

**Definition in Code:**
```typescript
// packages/schemas/src/locale.ts
export const SUPPORTED_UI_LOCALES = ['en', 'de', 'fr', 'es', 'zh', 'ja', 'ko'] as const;
...
Benutzer können ihre bevorzugte UI-Sprache auswählen. Die Einstellung wird persistent gespeichert.

### Sprach-Auflösung (Priority-Kette)

```
1. User.preferences.locale (DB) → eingeloggte Benutzer
2. NEXT_LOCALE Cookie → Gäste
3. Accept-Language Header → Browser-Präferenz
4. Default: 'en' (Englisch)
```
...
**Hinweis:** UI-Sprachen entsprechen den LocalizedString-Sprachen. Weitere Sprachen können hinzugefügt werden.

### Speicherung

| Benutzertyp | Speicherort | Persistenz |
|-------------|-------------|------------|
| Eingeloggt | `User.preferences.locale` (DB) | Permanent |
| Gast | `NEXT_LOCALE` Cookie | 1 Jahr |
...
**Locale-Auflösung:**
- `apps/web/src/i18n/request.ts` - Implementiert die Priority-Kette und Fehlerbehandlung

**Fehlerbehandlung für Sprachdateien:**
- Fehlerhafte Sprachdateien (JSON-Syntaxfehler etc.) führen nicht zum Absturz
- Fehler werden geloggt: `[i18n] Failed to load messages for locale "xx"`
- Automatischer Fallback zur Default-Sprache (Englisch)
- Fehlerhafte Sprachen werden gecached, um wiederholte Fehler zu vermeiden
- Im kritischen Fall (auch Default-Sprache defekt) wird leeres Messages-Objekt zurückgegeben

**Server Action:**
- `apps/web/src/app/actions/locale.ts` - `setLocale(locale)` setzt Cookie und DB

**UI-Komponente:**
- `apps/web/src/components/layout/locale-switcher.tsx` - Dropdown im Header

### Neue Sprachen hinzufügen

1. `SUPPORTED_UI_LOCALES` in `packages/schemas/src/locale.ts` erweitern
2. `LOCALES` Array in `locale-switcher.tsx` erweitern
3. Neue Message-Datei `apps/web/messages/{locale}.json` erstellen
4. Alle UI-Texte übersetzen
...
| UI-Sprachauswahl | ✅ | `apps/web/src/components/layout/locale-switcher.tsx` |
| Locale-Schemas | ✅ | `packages/schemas/src/locale.ts` |
| Server Action (setLocale) | ✅ | `apps/web/src/app/actions/locale.ts` |

### Unterstützte Sprachen

- **Englisch (en)** - Default-Sprache (Fallback)
- **Deutsch (de)**
- **Französisch (fr)**
- **Spanisch (es)**
- **Chinesisch (zh)**
